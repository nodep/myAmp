...
// just a dummy function to suppress GCC warning
...
int /*__attribute__((noreturn))*/ main(void)
{
	...
	// turn on the light-show by setting up the
	// SPI to communicate with the external shift register
	DDRB |= (_BV(FV_CLK_BIT) | _BV(LED_DATA_BIT) | _BV(LED_CLOCK_BIT) | _BV(LED_RST_BIT));
	PORTB |= _BV(LED_RST_BIT);
	
	SPCR = _BV(SPE) | _BV(MSTR);
	
	// setup the local timer
	TCCR0A = 0x00;
	TCCR0B = _BV(CS02) | _BV(CS00);		// 1024 prescler
	
	// keep FV-1 one happy by giving it a 32KHz clock
	// otherwise it will turn on the clip LED
	TCCR1A = _BV(COM1B0);	// output clock on B2
	TCCR1B = _BV(WGM12) |	// CTC
			 _BV(CS10);		// prescaler 1

	OCR1A = FV_FREQ_TO_CNT(FV_FREQ_DEFAULT);

    odDebugInit();
    DBG1(0x00, 0, 0);
    // wait for system to send new the firmware update
	// until the rotenc button is pressed
...
#endif
        } while (bootLoaderCondition()  &&  (usbConfiguration != 0  ||  ovcnt < 100));
    }
    leaveBootloader();
}
